...
runner <- function(choice){
    ...
    BestPrediction = c()
    stockVector = list()
    stockVector[[1]] = read.table(paste("stocks/",choice,".csv",sep = ""), sep=",", header=TRUE)
...
prediction_name = c("Arima auto on actual function","Arima auto on actual function","Neural Networks on actual function","Neural Networks on actual function",
                    "Holt winters on actual function","Arima auto on STL trend","Arima auto on STL trend",
                    "HoltWinters on STL trend","HoltWinters on STL trend","HoltWinters on STL trend","STL trend on Polynomial","STl on STL of polynomial",
                    "Neural Networks on STL trend","Neural Networks on STL trend",
                    "HoltWinters on STL trend","HoltWinters on STL trend ",
                    "Arima auto on polynomial trend","TSLM on actual function",
                    "TSLM on polynomial function","TSLM on STL trend","HoltWinters on Polynomial function",
                    "Neural Networks on Polynomial function","HoltWinters on STL trend",
                    "HoltWinters on polynomial trend","HoltWinters on actual trend")
